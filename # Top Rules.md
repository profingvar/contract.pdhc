...
**Rule 3.** readme.md contains the deployment plan numbered as 1.a.1, 1.a.2, etc.
...
**Rule 16.** Assume ownership of ports 9020–9023 on localhost. Kill all (9020–9030) before starting any program. Use only those ports. If practical, use the database on localhost:9020. Starting the database and other applications must be collected in a single bash script (./start.sh): kill previously used project ports (9020–9023), activate venv, start the DB and app; on Ctrl+C gracefully shut down and deactivate. See to that docker is started properly or is running when activating the application.
...
**Rule 22.** The future implementation on the server is fragile and all precaution must be taken to prevent disturbance of other services in the reverse proxy nor in the SSO
...
**Rule24** Use the pdhc_markdown_layout_standard.md and repo.css.md for all frontend designs
